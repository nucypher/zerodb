...
def _test_password():
    ...
    # ZEO.testing.  The server is using a server cert from ZEO.tests.
    password = 'sup3r'
    addr, stop = zerodb.server(init=dict(password=password))

    # Create an admin client.  Admin data aren't encrypted, so we use
    # a regular ZEO client.

    admin_db = ZEO.DB(addr,
                      ssl=ZEO.tests.testssl.client_ssl(client_cert=nobody_cert,
                                                       client_key=nobody_key),
                      credentials=dict(name='root', password=password)
                      )
    with admin_db.transaction() as conn:
